  ...
  return (
    <Card as="section" className="mt-6">
      ...
      {recommendations.length > 0 ? (
        <NewMusicRecommendationList recommendations={recommendations} />
      ) : null}
    </Card>
  );
}

export function NewMusicRecommendationList({
  recommendations
}: {
    ...
    <div className="mt-5 grid gap-3">
      {recommendations.map((recommendation) => (
        ...
        >
          ...
          <Link
            href={`/app/playlists/${encodeURIComponent(recommendation.playlistId)}`}
            className="mt-4 inline-flex"
          >
            <Button variant="glass" className="min-w-36">
              Open playlist
            </Button>
          </Link>
        </article>
